...
export function FourInARowBoard({ board, onPlay, canPlay, size = 56 }: FourInARowBoardProps) {
  const cell = size;
          ...
          row.map((cellVal, c) => {
            ...
            return (
              <div
                key={`cell-${r}-${c}`}
                style={cellStyle}
                className={`rounded-full ${color} flex items-center justify-center shadow-inner`}
